  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <Link href="/products/bldc">
              <Button variant="ghost" className="mb-6 hover:bg-primary/10 text-wrap-safe">
                <ArrowLeft className="me-2 h-4 w-4 rtl-flip" />
                {t('common:buttons.backToBldcProducts')}
              </Button>
            </Link>
            <h1 className="text-4xl md:text-5xl font-bold text-foreground mb-6 text-wrap-safe responsive-text">{t('products:productPages.bldcCeilingFan.title')}</h1>
      ...
      <div className="py-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <h2 className="text-3xl font-bold text-foreground text-center mb-12 text-wrap-safe responsive-text">{t('common:headings.bldcCeilingFanModels')}</h2>
          ...
          {products.map((product, index) => (
            <Card key={index} className="hover:shadow-lg transition-shadow duration-300 hover:scale-[1.02] group cursor-pointer border border-border/50 hover:border-primary/50">
              <CardHeader className="pb-4">
                ...
                />
                <CardTitle className="text-xl font-semibold text-foreground group-hover:text-primary transition-colors text-wrap-safe responsive-text line-clamp-safe" data-testid={`text-product-name-${index}`}>{product.name}</CardTitle>
      ...
      <div className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-foreground mb-6 text-wrap-safe responsive-text">{t('products:productPages.bldcCeilingFan.upgradeTitle')}</h2>
          <p className="text-lg text-muted-foreground mb-8 text-wrap-safe responsive-text">{t('products:productPages.bldcCeilingFan.upgradeDescription')}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact">
              <Button size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground text-wrap-safe">{t('products:productPages.bldcCeilingFan.contactExperts')}</Button>
            </Link>
            <Link href="/quote">
